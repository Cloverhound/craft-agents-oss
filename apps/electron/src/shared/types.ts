...
/**
 * Parse a defaultChatFilter string into a SessionFilter object.
 ...
 * - 'view:{viewId}'
 */
export const parseDefaultChatFilter = (filterStr: string | undefined): SessionFilter | null => {
  if (!filterStr) return null
  if (filterStr === 'allChats' || filterStr === 'allSessions') return { kind: 'allSessions' }
...
/**
 * Serialize a SessionFilter object to the string format used in defaultChatFilter.
 */
export const serializeChatFilter = (filter: SessionFilter): string => {
  switch (filter.kind) {
    case 'allSessions': return 'allChats'
