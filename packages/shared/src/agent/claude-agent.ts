...
export class ClaudeAgent extends BaseAgent {
  ...
  private async stopSessionRunner(): Promise<void> {
    if (this.sessionRunner) {
      debug("[ClaudeAgent] Stopping session runner");
  ...
  private initHeartbeatManager(): void {
    ...
    if (!this.heartbeatManager) {
      ...
      debug("[ClaudeAgent] Initialized heartbeat manager");
    }
  }
  ...
  private stopHeartbeatManager(): void {
    if (this.heartbeatManager) {
      ...
      debug("[ClaudeAgent] Stopped heartbeat manager");
    }
  }
  ...
  private async ensureSessionRunner(opts: {
    ...
    if (opts.forceNew && this.sessionRunner) {
      debug("[ClaudeAgent] Force stopping existing SessionRunner for fresh start");
      this.sessionRunner.forceStop();
      this.sessionRunner = null;
    }
  ...
  async *chat(
    ...
    try {
          ...
          PreToolUse: [{
            hooks: [async (input) => {
              ...
              if (input.tool_name === 'Skill') {
                const skillInput = (modifiedInput || toolInput) as { skill?: string };
                if (skillInput.skill) {
                  const skillSlug = skillInput.skill.includes(':')
                    ? skillInput.skill.split(':').pop()!
                    : skillInput.skill;
      ...
      const runner = await this.ensureSessionRunner({
        options,
        forceNew: _isRetry,
        ultrathink: this._ultrathinkOverride,
      });
  ...
  private buildTextPrompt(text: string, attachments?: FileAttachment[]): string {
    ...
    // Add credential registry state (Cloverhound custom feature)
